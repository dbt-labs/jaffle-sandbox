...
4. **Check column names** — `dft schema`, `dft schema -s`, or `dft query SOURCE SQL --describe` to verify.
...
| Assuming column names | Use `dft schema`, `dft schema -s`, or `dft query SOURCE SQL` to verify |
